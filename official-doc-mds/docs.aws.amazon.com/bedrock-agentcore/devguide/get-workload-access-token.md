...
**Topics**
+ [What is a workload access token?](#workload-access-token-overview)
+ [How Runtime and Gateway automatically obtain tokens](#how-runtime-gateway-obtain-tokens)
+ [How to manually retrieve workload access tokens](#manual-token-retrieval-patterns)
+ [Security controls for `GetWorkloadAccessTokenForUserId` API](#security-controls-getuserid-api)
...
<a name="token-retrieval-forjwt"></a>
...
<a name="token-retrieval-foruserid"></a>
 ...
 **Tradeoff:** The platform treats the userId as an opaque string and cannot verify it against an authenticated end-user identity. The security binding relies on the calling workload passing the correct userId and on IAM policies being scoped appropriately. See [Security controls for `GetWorkloadAccessTokenForUserId` API](#security-controls-getuserid-api) for recommended controls.
...
<a name="security-controls-getuserid-api-recommendations"></a>
