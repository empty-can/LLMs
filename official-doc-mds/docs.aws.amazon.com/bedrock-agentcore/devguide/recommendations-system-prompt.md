...
<a name="recommendations-system-prompt"></a>
...
## Code samples
<a name="start-sysprompt-rec-examples"></a>
...
+  **Optional encryption:** a KMS key (`--kms-key <arn>` — KMS key ARN for encrypting recommendation results)
  ...
  ```
   ...
   `run recommendation` starts an asynchronous job and returns only a `recommendationId` and an initial `PENDING` or `IN_PROGRESS` status. Add `--wait` to block until the recommendation reaches a terminal state. To retrieve the completed result later, see [Retrieve the result](#start-sysprompt-rec-retrieve).
  ...
  <a name="start-sysprompt-rec-retrieve"></a> **Retrieve the result:** 
...
## Request parameters
<a name="start-sysprompt-rec-params"></a>
...
<a name="start-sysprompt-rec-config"></a>
...
<a name="start-sysprompt-rec-evaluator"></a>
...
<a name="start-sysprompt-rec-input-modes"></a>
...
## Response
<a name="start-sysprompt-rec-response"></a>
...
<a name="start-sysprompt-rec-result"></a>
...
## Errors
<a name="start-sysprompt-rec-errors"></a>
