...
1. Configure a user pool domain so the hosted UI can issue authorization codes.

1. Create an app client for the user pool, generate a client secret, and enable the **Authorization code grant** OAuth2 flow.

1. Select the OAuth2 scopes your application requires.

1. Leave the **Allowed callback URLs** field empty for now — you will add the unique callback URL in Step 3.

1. Record the **Client ID** and **Client Secret**. You’ll need these values to configure the Cognito provider in AgentCore Identity.

1. Note your user pool’s domain prefix and user pool ID. You’ll use these to construct the authorization, token, and issuer endpoints in Step 2.
...
1. In the app client’s hosted UI configuration, add the `callbackUrl` value returned by `CreateOauth2CredentialProvider` to **Allowed callback URLs**.
