...
1.  **End user**: End users interact with the agent to achieve a specific goal. They can top up agent balance through crypto-based stablecoins or traditional payment methods like Credit cards (limited availability due to geographical restrictions), Debit cards, Apple Pay, Google Pay, or ACH so that agents can execute transactions on their behalf, with spending permissions, configurable spending limits, and full transparency into how their funds are used.

1.  **Merchant**: Merchants are API providers, content providers, and service operators who monetize their offerings through pay-per-use models. They set pricing, accept x402 payments, and deliver the requested content or service upon payment confirmation.
...
1.  ** [StripePrivy](https://docs.privy.io/wallets/overview/embedded) ** — Connects to Stripe with Privy wallet infrastructure.
