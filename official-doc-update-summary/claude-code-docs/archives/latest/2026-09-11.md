...
1. [**クラウド 3 プロバイダーが first-party セッションと同じ扱いになった**](../latest-detail/2026-09-11.md#1-クラウド-3-プロバイダーが-first-party-セッションと同じ扱いになった):  
...
2. [**タスク追跡ツールの既定が拒否リストから許可リストへ反転した**](../latest-detail/2026-09-11.md#2-タスク追跡ツールの既定が拒否リストから許可リストへ反転した):  
...
3. [**`claude plugin eval` の新設ページが宣言だけ先に届いた**](../latest-detail/2026-09-11.md#3-claude-plugin-eval-の新設ページが宣言だけ先に届いた):  
...
4. [**Claude apps gateway の持ち越し 3 節が到着した**](../latest-detail/2026-09-11.md#4-claude-apps-gateway-の持ち越し-3-節が到着した):  
...
5. [**v2.1.268 は 96 項目で、権限と秘匿情報まわりの修正が目立つ**](../latest-detail/2026-09-11.md#5-v21268-は-96-項目で権限と秘匿情報まわりの修正が目立つ):  
...
- [**evals でプラグインをテストする**](../latest-detail/2026-09-11.md#1-evals-でプラグインをテストする) ([日本語](https://code.claude.com/docs/ja/plugin-evals#requirements) / [English](https://code.claude.com/docs/en/plugin-evals#requirements)):  
  `claude plugin eval` によるプラグインの eval ハーネス。`llms.txt` と見出しマップには届いたが、**`llms-full.txt` には本文が未着**。内容はハイライト 3 で詳述。

## 大幅に更新されたページ

- [**Claude apps gateway 設定**](../latest-detail/2026-09-11.md#1-claude-apps-gateway-設定) ([English](https://code.claude.com/docs/en/claude-apps-gateway-config#send-the-rates-to-signed-in-clients)):  
  前回の持ち越し 3 節（`pricing` のクライアント配信・コレクターへの直接エクスポート・宛先失敗時のバックオフ）がすべて到着し、**実質 101 行（追加 81・削除 20）で今回最大**。内容はハイライト 4 で詳述。
- [**フック**](../latest-detail/2026-09-11.md#2-フック) ([English](https://code.claude.com/docs/en/hooks#worktreeremove)):  
