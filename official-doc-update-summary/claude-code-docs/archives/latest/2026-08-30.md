...
1. [**Bedrock ゲートウェイ経由のストリーミングで回避策の環境変数が推奨されなくなった**](../latest-detail/2026-08-30.md#1-bedrock-ゲートウェイ経由のストリーミングで回避策の環境変数が推奨されなくなった):  
...
2. [**ポリシーヘルパーの失敗条件と挙動が独立した節にまとまった**](../latest-detail/2026-08-30.md#2-ポリシーヘルパーの失敗条件と挙動が独立した節にまとまった):  
...
3. [**auto モードが使えない理由にサーバー側の一時停止が加わった**](../latest-detail/2026-08-30.md#3-auto-モードが使えない理由にサーバー側の一時停止が加わった):  
...
4. [**パースできない複合コマンドは許可ルールにも自動バックグラウンド化にも乗らない**](../latest-detail/2026-08-30.md#4-パースできない複合コマンドは許可ルールにも自動バックグラウンド化にも乗らない):  
...
5. [**クラウド環境の API 認証情報を追加できるのは Owner だけと訂正された**](../latest-detail/2026-08-30.md#5-クラウド環境の-api-認証情報を追加できるのは-owner-だけと訂正された):  
