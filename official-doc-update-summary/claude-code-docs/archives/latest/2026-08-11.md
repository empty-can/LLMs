...
- サーバー管理設定に **承認の記憶** の節が加わりました。承認の記録単位が資格情報によって変わり、`/login` の claude.ai ログインなら組織ごとに 1 件（最後に承認したアカウントが保持）、それ以外の資格情報なら配信された設定ごとに 1 件（キャッシュと一緒に保持され、`/logout` で消える）です — [日本語](https://code.claude.com/docs/ja/server-managed-settings#approval-memory) / [English](https://code.claude.com/docs/en/server-managed-settings#approval-memory)
...
- セルフホスト環境のテストページに **`--environment` のディスパッチ挙動** の節が新設されました。非対話実行ではセッションを作って ID とリンクを表示して終了し、ターミナルからはアタッチ済みの対話クラウドセッションが始まること、`remote.defaultEnvironmentId` より優先されること、`--resume` `--continue` `--teleport` `--session-id` `--init-only` と併用できないことが書かれています — [日本語](https://code.claude.com/docs/ja/self-hosted-environments-testing#environment-dispatch-behavior) / [English](https://code.claude.com/docs/en/self-hosted-environments-testing#environment-dispatch-behavior)
...
- ネットワーク要件の一覧に `registry.npmjs.org` が追加されました。プラグインのインストール（npm ソースのパッケージ取得とプラグインの Node.js 依存インストール）、`npx` 起動の MCP サーバー、npm / bun による Claude Code 自体のインストールが用途です — [日本語](https://code.claude.com/docs/ja/network-config#network-access-requirements) / [English](https://code.claude.com/docs/en/network-config#network-access-requirements)
