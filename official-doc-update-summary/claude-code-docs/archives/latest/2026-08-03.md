...
- routines の「`/schedule` が Unknown command を返す」トラブルシュートで、認証方法によってメッセージが違うことが明記されました。Console API キーで認証している場合は `/schedule is available with Claude for Enterprise — ask your admin about migrating from API-key access` が表示され、クラウドプロバイダのログインでは従来どおり `Unknown command: /schedule` になります。フィーチャーフラグ取得を止める環境変数を設定している場合など、それ以外の原因では引き続き `Unknown command: /schedule` です — [日本語](https://code.claude.com/docs/ja/routines#schedule-returns-unknown-command) / [English](https://code.claude.com/docs/en/routines#schedule-returns-unknown-command)
