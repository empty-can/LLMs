...
- hooks リファレンスの共通入力フィールド表で、`transcript_path` の説明に、トランスクリプトファイルは非同期に書かれるためフック発火時点で当該ターンの最新メッセージをまだ含まない場合がある旨と、現ターンの最終アシスタントテキストが必要なフックは `transcript_path` を読むのではなく Stop / SubagentStop の `last_assistant_message` を使うべき旨が追記された。 — [日本語](https://code.claude.com/docs/ja/hooks#common-input-fields) / [English](https://code.claude.com/docs/en/hooks#common-input-fields)
