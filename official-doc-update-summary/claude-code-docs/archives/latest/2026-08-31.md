...
- `CLAUDE_CODE_AUTO_COMPACT_WINDOW` の説明に、**実効のウィンドウはモデルのコンテキストウィンドウでも上限が掛かる**ことが加わりました — [日本語](https://code.claude.com/docs/ja/env-vars#variables) / [English](https://code.claude.com/docs/en/env-vars#variables)
...
- `/usage-credits` について、**セルフサービスの Enterprise 組織・Enterprise トライアル・AWS Marketplace 経由で請求される Enterprise 組織では v2.1.248 以降が必要**で、それより前のバージョンは `Unknown command: /usage-credits` で拒否する、と加わりました — [日本語](https://code.claude.com/docs/ja/costs#add-usage-credits-to-your-subscription) / [English](https://code.claude.com/docs/en/costs#add-usage-credits-to-your-subscription)
...
- 設定ページで、`--effort` フラグが対応するキーとして `effortLevel` に加えて `modelSettings` が挙げられ、セッション内コマンドの説明も「`/model` は既定として保存し、**自分のマシンでの `/effort` は使用中のモデルの既定としてレベルを保存する**」に改まりました（詳細はハイライト 1 参照）— [日本語](https://code.claude.com/docs/ja/settings#change-a-setting-for-one-session) / [English](https://code.claude.com/docs/en/settings#change-a-setting-for-one-session)
...
- Agent SDK（TypeScript）の `RemoteTrigger` ツールの説明から、フィーチャーフラグの取得をオフにすると使えないという条件が外れました（ツールリファレンスの同じ行と対になる変更です）— [日本語](https://code.claude.com/docs/ja/agent-sdk/typescript#remotetrigger) / [English](https://code.claude.com/docs/en/agent-sdk/typescript#remotetrigger)

## 新着情報

- [**2026年08月17日～21日(Week 34)**](../latest-detail/2026-08-31.md#2026年08月17日21日week-34) ([日本語](https://code.claude.com/docs/ja/whats-new/2026-w34) / [English](https://code.claude.com/docs/en/whats-new/2026-w34)):  
