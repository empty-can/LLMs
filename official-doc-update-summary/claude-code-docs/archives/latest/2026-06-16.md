...
1. [**エージェントチームのワークフロー刷新**](../latest-detail/2026-06-16.md#1-エージェントチームのワークフロー刷新):  
...
2. [**権限ルールが入力パラメータにマッチ可能に**](../latest-detail/2026-06-16.md#2-権限ルールが入力パラメータにマッチ可能に):  
...
3. [**フッターリンクバッジの追加**](../latest-detail/2026-06-16.md#3-フッターリンクバッジの追加):  
...
4. [**ネストした設定ディレクトリのサポート拡大**](../latest-detail/2026-06-16.md#4-ネストした設定ディレクトリのサポート拡大):  
...
5. [**アドバイザーのペアリング検証がクライアント側に**](../latest-detail/2026-06-16.md#5-アドバイザーのペアリング検証がクライアント側に):  
...
- [**エージェントチーム（agent-teams）の刷新**](../latest-detail/2026-06-16.md#1-エージェントチームのワークフロー刷新) ([English](https://code.claude.com/docs/en/agent-teams#architecture)):  
...
**新機能**
- Amazon Bedrock の認証情報ヘルパー出力に任意の `Expiration` フィールドが追加された。v2.1.176 以降、コマンドが有効な ISO 8601 の `Expiration` を返すと、その 5 分前まで認証情報がキャッシュされる（無い場合や旧バージョンでは従来どおり 1 時間キャッシュ）。 — [日本語](https://code.claude.com/docs/ja/amazon-bedrock#2-configure-aws-credentials) / [English](https://code.claude.com/docs/en/amazon-bedrock#2-configure-aws-credentials)
...
- Agent SDK に 2 つのメッセージ型が追加された。`SDKInformationalMessage`（ループからの非エラーのステータスバナーや hook フィードバックを `level` 付きで運ぶ）と `SDKWorkerShuttingDownMessage`（ホスト終了や Remote Control 切断によるワーカーの正常終了理由 `reason` を通知）。あわせて `SystemMessage` の `subtype` に `informational` / `worker_shutting_down` が加わった。 — [日本語](https://code.claude.com/docs/ja/agent-sdk/typescript#sdkmessage) / [English](https://code.claude.com/docs/en/agent-sdk/typescript#sdkmessage)
