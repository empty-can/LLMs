...
- フルスクリーンレンダリングでの高速スクロール時のマウスホイール加速をオフにできる `wheelScrollAccelerationEnabled` 設定が追加された（v2.1.174）。 — [日本語](https://code.claude.com/docs/ja/settings#available-settings) / [English](https://code.claude.com/docs/en/settings#available-settings)
...
- `opusplan` の Plan Mode の Opus フェーズが、自動 1M アップグレード対象のプランでは 1M コンテキストを受け取るようになり、`opusplan[1m]` で両フェーズに 1M を強制できることが明記された（従来は 200K 固定と記載）。 — [日本語](https://code.claude.com/docs/ja/model-config#opusplan-model-setting) / [English](https://code.claude.com/docs/en/model-config#opusplan-model-setting)
