...
- [スキルで Claude を拡張する - Claude Code Docs (日本語)](https://code.claude.com/docs/ja/skills#frontmatter-reference)
- [Extend Claude with skills - Claude Code Docs (English)](https://code.claude.com/docs/en/skills#frontmatter-reference)
...
- [日本語](https://code.claude.com/docs/ja/zero-data-retention) / [Zero data retention - Claude Code Docs (English)](https://code.claude.com/docs/en/zero-data-retention)
...
今回の軽微な更新は changelog への v2.1.169（大型リリース）追加が中心で、ZDR（ゼロデータ保持）の適格要件を明確化するデータ関連ページの追記も入りました。

**新機能**
- `claude agents` に、完了済みセッションも含める `--all` フラグと、各セッションの `id` / `state` フィールドが追加されました。

**機能改善**
- ターンの進行中でも `/workflows` がすぐ開くようになりました。
- `TaskCreate` の信頼性を改善しました（不正な入力は自動修復され、未ロードのツールに対する検証エラーにスキーマが含まれるように）。
- 組織が API キー認証を無効化している場合のエラーメッセージを、現在の API キーの出所に基づくガイダンス表示へ改善しました。
- 応答ストリーミング中とスピナーアニメーション中の CPU 使用率を削減しました。
- Vertex / Foundry でデフォルト 5 分のアイドルタイムアウトを復活させ、停止したストリームが無限にハングせず中断されるようにしました（`API_FORCE_IDLE_TIMEOUT=0` で無効化可能）。
- リモート管理設定に無効なエントリがあるとき、ペイロード全体を黙って破棄せず、残りの有効なポリシーを適用して検証エラーを表面化するようにしました。
- バックグラウンドセッションが retire→wake をまたいで `--ide` / `--chrome` / `--bare` / `--remote-control` などのフラグを保持するようになり、再生成時の状態検証も強化されました。
- バックグラウンドセッションに、worktree に入るまで共有チェックアウトの編集がブロックされる旨を伝え、`EnterWorktree` 前の無駄な編集拒否を避けるようにしました。
- 「CLAUDE.md is too long」警告のしきい値が、モデルのコンテキストウィンドウに応じてスケールするようになりました。
- スラッシュコマンドメニューのスキルタグの色コントラストを改善しました。
- 支払い方法のない Apple / Google 課金のサブスクライバーがプロモクレジットを請求する際、支払い方法の追加場所を案内するようにしました。
- 複数の並行セッションを実行しているとき、`claude agents` を提案するヒントを追加しました。
- データ利用ページの保持に関する記述が、ZDR は適格アカウント向け・標準プラン非同梱で適格性確認後に有効化される、という表現へ更新されました（詳細はハイライト 5 参照）。 — [English](https://code.claude.com/docs/en/data-usage#data-retention)
- 組織向けセットアップページのデータ取り扱い概要表の ZDR 行が「適格アカウント向け（available to qualified accounts）」表記へ更新されました。 — [English](https://code.claude.com/docs/en/admin-setup#review-data-handling)

**バグ修正**
- 長い入力行で折り返した行を超えてコマンド履歴へジャンプしていた上下矢印を修正しました（各表示行を先に移動し、履歴呼び出しは近い端から入るように）。
- エンタープライズの管理 MCP ポリシー（`allowedMcpServers` / `deniedMcpServers`）が、再接続時・IDE で入力した構成・インストール後最初のセッション中の `--mcp-config` サーバー・リモート設定の読み込み前に強制されない問題を修正しました（リモート設定を持たない組織での遅いコールドスタートも修正）。
- claude.ai 認証情報でログインした macOS ユーザーで、各ターン開始時に約 30〜50ms の UI 停止が起きる問題を修正しました。
- Windows で `claude -p` がスラッシュコマンド / スキルのスキャン待ちの間に遅くなる・ハングしたように見える問題を修正しました（2.1.161 のリグレッション）。
- セッション再開時に OAuth トークン更新が同時に起きると、リモートコントロールが「reconnecting」で止まる問題を修正しました。
- バックグラウンドの git コマンドがキャッシュ済み認証情報なしで実行されたとき、Windows の起動時に Git Credential Manager の「Connect to GitHub」ポップアップが出る問題を修正しました。
- カスタムステータスラインを使うユーザーでフッターのヒント（例: 「esc to interrupt」）が表示されない問題を修正しました。
- ワーカーが死んだリモートセッションに再アタッチするたび、待機中だった古い権限・ダイアログのプロンプトが再表示される問題を修正しました。
- `claude agents --json` がブロック中・ディスパッチ直後のバックグラウンドセッションを省く問題を修正しました。
- Windows Terminal の WSL でエージェントから戻ったあと、エージェントビューに古い / 文字化けしたフレームが残る問題を修正しました。
- 事前ウォームされたワーカーにディスパッチされたバックグラウンドエージェントが、プロジェクトレベル設定の `env` 値（例: `ANTHROPIC_MODEL`）を無視する問題を修正しました。
- Windows で MCPB プラグインのキャッシュが誤って無効化され、不要な再展開が起きる問題を修正しました。
- プラグインの `.in_use` PID ロックファイルが際限なく蓄積する問題を修正しました（クラッシュしたセッションの古いマーカーは 1 日 1 回掃除されるように）。
- 信頼されていないプロジェクト設定が、信頼確認なしに OTEL クライアント証明書のパスを設定できる問題を修正しました。
- Windows の自動更新が、`claude.exe` が別プロセスに保持されている場合、セッション内での再試行を停止するようになりました。

**その他**
- changelog ページに v2.1.169（2026年06月08日）のリリースエントリが追加されました（上記の機能改善・バグ修正の多くを含む大型リリース。ハイライトの `--safe-mode` / `/cd` / `disableBundledSkills` も同リリース由来）。
