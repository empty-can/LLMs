...
**その他**
- model-config ページの「Work with Fable 5」セクションで、Fable 5 紹介ページへの外部リンク先が `introducing-claude-fable-5` から `introducing-claude-fable-5-and-claude-mythos-5` に更新された（リンク先 URL の変更のみで、本文の内容変更はなし）。 — [日本語](https://code.claude.com/docs/ja/model-config#work-with-fable-5) / [English](https://code.claude.com/docs/en/model-config#work-with-fable-5)
