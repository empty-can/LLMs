...
- [権限モードを選ぶ - Claude Code Docs (日本語)](https://code.claude.com/docs/ja/permission-modes#enable-auto-mode-on-bedrock-vertex-ai-or-foundry)
...
- [Claude Code のプロンプトキャッシュの仕組み - Claude Code Docs (日本語)](https://code.claude.com/docs/ja/prompt-caching#turning-on-fast-mode)
- [How Claude Code uses prompt caching - Claude Code Docs (English)](https://code.claude.com/docs/en/prompt-caching#turning-on-fast-mode)
...
- [Explore the context window - Claude Code Docs (English)](https://code.claude.com/docs/en/context-window#when-your-context-fills-up)
...
- [動的ワークフローでサブエージェントを大規模にオーケストレーションする - Claude Code Docs (日本語)](https://code.claude.com/docs/ja/workflows#pass-input-to-a-saved-workflow)
- [Orchestrate subagents at scale with dynamic workflows - Claude Code Docs (English)](https://code.claude.com/docs/en/workflows#pass-input-to-a-saved-workflow)
...
今回の軽微な更新は、Agent SDK リファレンスの拡充と、トラブルシューティング事例・changelog 表記の調整です。

**新機能**
- TypeScript Agent SDK リファレンスに、新しいメッセージ型 `SDKCommandsChangedMessage` が追加されました。サブディレクトリへ移動してスキルが検出されるなど、セッション途中で利用可能なコマンド集合が変化したときに発行され、`commands` 配列が更新後の全リストになります。初期化時のスナップショットを返す `supportedCommands()` の再呼び出しでは反映されません。 — [日本語](https://code.claude.com/docs/ja/agent-sdk/typescript#sdkcommandschangedmessage) / [English](https://code.claude.com/docs/en/agent-sdk/typescript#sdkcommandschangedmessage)

**機能改善**
- フック（Agent SDK）のレシピ見出しが「Filter with regex matchers」から「Filter with multi-tool matchers」へ改称され、複数ツールマッチャーで 1 つのコールバックを共有する例が示されました。パイプ区切りの完全一致リスト（`Write|Edit|Delete`）・正規表現（`^mcp__`）・マッチャー省略（全ツール対象）の 3 つのスコープを使い分けられます。 — [日本語](https://code.claude.com/docs/ja/agent-sdk/hooks#filter-with-multi-tool-matchers) / [English](https://code.claude.com/docs/en/agent-sdk/hooks#filter-with-multi-tool-matchers)
- トラブルシューティングに、VS Code・Cursor・Devin Desktop の統合ターミナルで文字が箱・かすれ・誤ったグリフとして描画されるケースが追加されました。多くはターミナルの GPU レンダラーが原因で、`/terminal-setup` の実行（`terminal.integrated.gpuAcceleration` を `"off"` に設定）で対処できます。 — [日本語](https://code.claude.com/docs/ja/troubleshooting#garbled-or-corrupted-text-in-an-editor-s-integrated-terminal) / [English](https://code.claude.com/docs/en/troubleshooting#garbled-or-corrupted-text-in-an-editor-s-integrated-terminal)
...
- 前回サマリ(ライト版): [../latest/2026-06-02_1125.md](../latest/2026-06-02_1125.md)
- 前回サマリ(詳細版): [../latest-detail/2026-06-02_1125.md](../latest-detail/2026-06-02_1125.md)
