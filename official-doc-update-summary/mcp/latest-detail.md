...
1. [**MCP の意義を説明する箇所の文言整理**](#1-mcp-の意義を説明する箇所の文言整理):  
...
## 1. MCP の意義を説明する箇所の文言整理

今回の対象期間で本文に差分が出たのは「What is the Model Context Protocol (MCP)?」ページ 1 件のみで、変更箇所は「Why does MCP matter?」節の箇条書き 2 行です。この節は MCP の利点を開発者・AI アプリケーション／エージェント・エンドユーザーという 3 つの立場から 1 行ずつ述べたもので、このうち開発者向けの 1 行は変更されていません。

変更は次の 3 点です。1 つ目は指示対象の明確化で、AI アプリケーション／エージェント向けの行が `MCP provides access to an ecosystem of data sources, tools and apps which will enhance capabilities…` から `MCP gives them access to an ecosystem of data sources, tools and apps, which enhances their capabilities…` になり、誰がアクセスを得て誰の能力が高まるのかが `them` / `their` で明示されました。あわせて関係代名詞の前にカンマが置かれ、直前の名詞ではなく前段全体を受ける非制限用法であることがはっきりしています。2 つ目は同じ行の時制で、`will enhance` / `improve` という未来形が `enhances` / `improves` の現在形に改められました。3 つ目は人称で、エンドユーザー向けの行の `agents which can access your data and take actions on your behalf` が `agents that can access user data and take actions on the user's behalf` になり、読み手を指していた二人称がユーザーを指す三人称に統一されています（関係代名詞も `which` から `that` へ）。

いずれも文法・文体上の調整であり、MCP の位置づけや、それぞれの立場にとっての提供価値についての記述内容そのものは変わっていません。

- [What is the Model Context Protocol (MCP)? - MCP Docs](https://modelcontextprotocol.io/docs/2026-07-28/getting-started/intro#why-does-mcp-matter)
