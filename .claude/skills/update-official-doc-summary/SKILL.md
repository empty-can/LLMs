...
> **テンプレート内のメンテナー向けコメントの扱い**: テンプレ先頭の「メンテナー編集ガイド」コメントと `intro-style:` コメントは**テンプレ専用の指示**であり、生成する `$LATEST_DETAIL` には**写さない**(出力から省く)。ただし `intro-style:` コメントに文体・トーンの指示が書かれていれば、その指示に沿って `{{OVERALL_SUMMARY_INTRO}}` の言い回しを整える。
>
> **冒頭コピペ用エリアの省略**: テンプレートから `<!-- light:summary:start -->` 〜 `<!-- light:summary:end -->` マーカー(および `{{OVERALL_SUMMARY_INTRO}}` / `{{OVERALL_SUMMARY_BULLETS}}`)が削除されている場合は、概要の生成・埋め込みを**スキップ**する(埋め先が無いだけでエラーにはしない。`derive_light.py` も summary 不在を許容する)。この場合、手順 7 の「概要・ハイライト件数整合性」チェックは対象外とする。
>
> **見出しはテンプレから継承**: 各セクションの `## 見出し` 文言・並び順はテンプレート側で決まり、`derive_light.py` がライト版へ継承する。生成時はテンプレの見出し文言・セクション順を**そのまま尊重**し、勝手に並べ替え・改称しない(メンテナーがテンプレで決めた構成を保つ)。
...
| `{{MINOR_UPDATES}}` | 軽微更新を **新機能 / 機能改善 / バグ修正 / その他** の 4 分類で整理(後述「軽微な更新の分類」参照) |
...
- **形式**: 概要全体を ```` ```markdown ```` フェンスで囲む(Mattermost / Wiki へそのままコピペできるよう **引用書式 `>` は使わない**)。フェンス内は「冒頭 1〜2 文 + 空行 + `主要なものを以下に挙げます。` + 空行 + 番号付き箇条書き」。各行は `1. <項目>` 〜 `N. <項目>`(先頭に `>` を付けない)。番号・順序はハイライト(`1.`〜`N.`)と一致させる。フェンスは `<!-- light:summary:start -->` / `<!-- light:summary:end -->` マーカーの内側に置く(テンプレート参照)
...
| `{{MINOR_UPDATES}}` | 4 分類(新機能 / 機能改善 / バグ修正 / その他)で整理。後述「軽微な更新の分類」参照 |
...
#### 軽微な更新の分類（新機能 / 機能改善 / バグ修正 / その他）

`## 軽微な更新`(`{{MINOR_UPDATES}}`)は、軽微更新の各項目を内容の性質で **4 分類**に振り分けて整理する。分類見出しは **太字ラベル**(`**新機能**` / `**機能改善**` / `**バグ修正**` / `**その他**`)とし、その配下に `-` 箇条書きを置く。**該当項目が無い分類は省略**してよい。先頭に、対象 changelog リリースや更新ページ件数を述べる導入文を 1 文置いてよい。

| 分類 | 入るもの |
|---|---|
| **新機能** | 新規のコマンド・フラグ・UI 要素・能力など、これまで存在しなかったもの |
| **機能改善** | 既存挙動の拡張・パフォーマンス改善・対応範囲拡大・ドキュメント記述の明確化 |
| **バグ修正** | 不具合の修正 |
| **その他** | 上記に当てはまらない軽微な変更(リンク先 URL の変更、字句修正など) |

- **公式ページの有無を問わず分類に入れる**: 対応する ja/en 公式ページが既にある軽微更新も、いずれかの分類の 1 項目として箇条書きに含める。ページがある項目は項目末尾に ` — [日本語](url-ja) / [English](url-en)` を付す(リンク形式は「末尾参考リンクのセクションアンカー化」に従い、**該当セクションのアンカー付き**とする)。
- **changelog 由来でページが無い項目**: リンクを付けず本文のみ。**changelog ページの URL は使わない**(changelog リンク不使用ポリシー)。
- **ハイライトに昇格済みの項目**: 分類内では「(詳細はハイライト N 参照)」と一言で示し、本文を二重掲載しない。
- **1 項目 1 bullet**: 読点(、)で連結した長大な 1 文にしない。1 つの changelog に多数の修正が含まれる場合も、各修正を独立した bullet で列挙する(本文が「信頼性改善」等の汎用文のみのリリースは 1 行で可)。
- **複数リリースを含む場合**: 各 bullet にバージョンを併記して識別可能にする(例:「…を修正(v2.1.168)」)。単一リリースのみのときは導入文で 1 度述べれば各 bullet への併記は不要。
...
```
- [<日本語タイトル> - Claude Code Docs (日本語)](https://code.claude.com/docs/ja/<path>#<section-id>)
- [<English title> - Claude Code Docs (English)](https://code.claude.com/docs/en/<path>#<section-id>)
```
...
- **各リンクは該当セクションのアンカー `#<section-id>` を付ける**(ページ全体ではなく、その内容が言及されているセクションを指す)。アンカーの決め方と、**ja ページが変更を反映していない場合に ja 行を省略する**判断は後述「末尾参考リンクのセクションアンカー化」に従う
...
末尾参考リンク・bullet 末尾リンクの ja/en URL は、ページ全体ではなく **そのハイライト/changelog 内容が実際に言及されているセクションのアンカー** (`<page-url>#<section-id>`) を指す。読み手が該当箇所へ直接到達できるようにするため。

**en アンカーの決定**:
- `$INPUT_LLMS_FULL` 内でその変更を記述している見出し(`## <English heading>` / `### <English heading>`)を特定する。
- アンカー id は Mintlify が付与する**明示的な英語スラッグ**で、見出しテキストの GFM スラッグ化(小文字化・スペース→ハイフン・英数字とハイフン以外を除去)と一致する。例: `### Submit your plugin to the community marketplace` → `#submit-your-plugin-to-the-community-marketplace`
- 1 つの変更が複数セクションにまたがる場合は、最もよく説明している**単一**セクションを選ぶ。該当見出しが無く本文に埋もれている場合のみアンカー無しのページ URL にフォールバックする

**ja アンカーと省略判断(翻訳ラグ対応)**:
- code.claude.com(Mintlify)は ja 見出しにも **en と同一の英語 id** を付与する。ja に該当セクションがあれば ja リンクのアンカーも en と同じ `#<section-id>` を使う
- ただし ja がその変更を反映済みとは限らない。次のいずれかなら **ja リンクを記載しない**(en のみ残す):
  - ja ページに該当セクション(同一 id)が**存在しない**
  - 該当セクションはあるが**内容が変更を反映していない**(旧記述のまま=翻訳が未追従)
- 反映判定は en のみの `$INPUT_LLMS_FULL` ではできないため、対象 ja ページを **WebFetch で取得**し、セクションの有無と内容(変更後になっているか)を確認する。判定不能なら安全側で ja を省略する(古い内容へ誤誘導しない)
- ja を省略した項目は末尾を `— [English](<en-url>#<id>)` の **en 単独**にする(`[日本語](…) / ` 部分を出力しない)

> changelog でしか言及がない変更(対応通常ページにセクションが無い)は従来どおりリンク省略(changelog URL は使わない)。本アンカー化は対応通常ページが既にある場合のみ適用する。言語サブパスが無いサイト(mcp)は単一 URL に `#<section-id>` を付け、ja/en 出し分けは生じない。
...
- [ ] セクションアンカー: 対応通常ページがある末尾参考リンク・bullet 末尾リンクの各 URL に該当セクションの `#<section-id>` が付いている(本文に埋もれ見出しが無い場合のみアンカー無し)。en の id は `$INPUT_LLMS_FULL` の見出し GFM スラッグと一致
- [ ] ja 翻訳ラグ省略: ja ページに該当セクションが無い/内容が変更未反映の項目は ja リンクを省略し en 単独になっている(WebFetch で ja ページを確認済み)
