...
- 起動: タスクスケジューラ `CC-DocJaFollowBot`（毎日 15:30 / Interactive logon / ExecutionTimeLimit PT1H）
...
```mermaid
flowchart TD
    A["タスクスケジューラ 15:30"] --> B["手順1 前提チェック<br/>最大20分・120秒間隔でポーリング"]
...
生成 bot は 15:00 起動だが所要時間が数分〜30 分超とばらつくため、固定 30 分オフセットでは
...
| 症状 | 原因 | 防御・対処 |
|---|---|---|
| 15:30 台に「未コミットの変更があります」で FAILURE | 生成 bot がまだ走っている（レース） | 20 分のポーリング待機で吸収（`c826238`）。翌日は自己回復する |
...
- `linkless`（公式ページ未作成）と `manual`（自動判定不可）は日次バッチでは解消しない。
